...
- **i2t R@1: a genuinely different, striking picture — but apply the same noise-floor standard used to discount the t2i result.** 12 of 15 non-baseline pairs clear the project's `mean/SEM ≥ 2` significance bar, all **positive**. Of those 12, **10 also exceed the ~0.1–0.7 R1 noise floor in magnitude** (up to +2.30 R1) — several substantially so (`dinov2:minilm` +2.30, `dinov2:bge` +1.97, `vit_sup:clip_txt` +1.77, `clip_img:bge` +1.43). The remaining **2 (`clip_img:e5` +0.60, `vit_sup:bge` +0.73) are statistically flagged but sit within/at the edge of the noise-floor band** — the same standard that discounted `dinov2:minilm`'s t2i result applies here too, so these two are reported as flagged-but-marginal rather than folded into the "striking" majority uncritically. **The current default — sourcing the buddy graph from CLIP features, the same encoder used for the training backbone — is still one of the *worse* choices among the 16 tested for i2t retrieval**, on the strength of the 10 pairs that clear both bars. Most alternatives beat it, several strongly, all seed-replicated (3/3 seed wins for every significant positive pair, including the 2 marginal ones). Two pairs go the other way, both pairing the "e5" text encoder with a non-CLIP vision encoder: `siglip_v:e5` (−0.37, not significant) and `vit_sup:e5` (−0.80, significant negative) — but `dinov2:e5` is positive and significant (+0.93), so this is pair-specific, not "e5 is uniformly bad."
...
**Baseline arm construction — not bit-identical to the historical default path.** The sweep script (`scripts/run_buddy_init_encoder_ablation.sh`) passes `+train.buddies.encoder_pair=clip_img:clip_txt` even for the baseline arm, so that all 16 arms (including the "baseline") share one code path (the `feature_override` branch added for this experiment) for internal consistency — rather than the baseline silently taking the pre-existing, un-overridden default path that every prior CoSiR experiment (C5–C7) used. These two paths are logically equivalent (both ultimately feed CLIP's own image/text features into the same `compute_buddy_init` call) but are **not bit-identical**, because the spectral solver (`eigen_solver="auto"`, ARPACK/LOBPCG-backed) is not guaranteed to reproduce identical eigenvectors run-to-run even on the same input matrix, and the two paths construct/order the input arrays slightly differently before that call. This was checked empirically with the two Task 3 smoke runs already on disk at `res/CoSiR_buddy_encoder_ablation/_smoke/{default,override}/template_embeddings/embeddings.npy` (2-epoch RedCaps-150k runs, one with no override, one with `+train.buddies.encoder_pair=clip_img:clip_txt`, both `seed=1`, 150,000 rows, `embedding_dim=16`, identical sample-id order). Independently re-loading and comparing these two real, saved template tensors: **max abs diff 2.00, mean row-cosine similarity 0.588**, and a per-dimension Pearson correlation across all 16 spectral dimensions found **13 of 16 dimensions matching at |r| ≥ 0.91** (2 of those clearly sign-flipped, at r = −0.9996 and −0.9998 — a benign, expected artifact of spectral eigenvectors having an arbitrary sign) and **3 dimensions genuinely unstable** between the two paths (r = 0.76, 0.53, and −0.81 — the last one both weak *and* sign-flipped). **Conclusion: the two paths recover the same spectral subspace up to sign, with a handful of near-degenerate directions that move between runs** — reassuring for the ablation's internal validity (the "baseline" arm is a faithful stand-in for the historical default), but the two paths are not numerically interchangeable at the individual-embedding-dimension level, and this was not previously disclosed in this report.
...
**Read: a genuinely different, striking picture, with the same noise-floor standard applied as t2i.** 12 of the 15 non-baseline pairs clear `|mean/SEM| ≥ 2`, all 12 **positive**, and every one of those 12 wins 3/3 seeds — this is not a noisy, cherry-picked subset, it's the large majority of the grid, seed-replicated across the board. But statistical significance alone is not the bar this report uses elsewhere: the same ~0.1–0.7 R1 noise floor that discounted `dinov2:minilm`'s t2i result (statistically flagged, magnitude below the floor) must also be checked against the i2t magnitudes. Of the 12 significant pairs, **10 exceed the noise floor in magnitude** (0.73–2.30 R1) and 2 (`clip_img:e5` +0.60, `vit_sup:bge` +0.73) sit within/at the top edge of the band — flagged by the statistical test but marginal in practical magnitude, the same category `dinov2:minilm` fell into for t2i. **The CLIP-sourced default (`clip_img:clip_txt`) is still one of the worse choices among the 16 pairs tested for i2t retrieval**, on the strength of the 10 pairs that clear both bars — most alternatives beat it, several by more than 1.5 R1 pts (`dinov2:minilm` +2.30, `dinov2:bge` +1.97, `vit_sup:clip_txt` +1.77, `clip_img:bge` +1.43).
...
- **i2t:** far more than "one or more" — 12 of 15 non-baseline pairs beat the CLIP-sourced default at the `mean/SEM ≥ 2` significance bar, seed-replicated; 10 of those 12 also clear the ~0.1–0.7 R1 noise floor in magnitude, several by a wide margin. `dinov2:minilm`, `dinov2:bge`, `vit_sup:clip_txt`, and `clip_img:bge` are strong candidates for a stronger initializer to lead the paper with on this axis, pending the caveats below. (`clip_img:e5` and `vit_sup:bge` are statistically significant but noise-floor-marginal — not strong candidates on their own.)
...
- **"Seed-replicated" samples training stochasticity only, not graph-construction/solver variance.** The buddy-graph init is computed once per encoder pair and shared deterministically across all 3 seeds (the established convention in this project, matching Experiments 1 and 10) — the reported seed-replication samples training stochasticity only, not graph-construction/spectral-solver draw variance. This convention is correct and symmetric between the baseline and every treatment arm, not a bias toward either. Given the Method section's evidence above (a couple of spectral dimensions can be draw-unstable between logically-equivalent runs of the solver), a follow-up rebuilding one pair's template under a second solver seed would be a cheap way to check whether this matters for the reported deltas.
