...
- All 6 runs use `combine_side="img"` — the condition is fused only into the image embedding in every run this cross-reference draws on. This means the graph-topology explanation above and the "training can only move the image side" architectural explanation are confounded here: both predict the same `img_only_only` > `bridge`/`txt_only_only` > `neither` ordering, and this experiment cannot distinguish them. A `combine_side="txt"` replication (not run here) would: if the effect flips to concentrate on `txt_only_only` instead, the architectural explanation dominates; if it stays on `img_only_only` regardless of `combine_side`, the graph-topology explanation is favored.
