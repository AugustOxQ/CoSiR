...
**A second, more mechanistic candidate explanation, not mutually exclusive with the graph-topology one above:** every run in this cross-reference (11.1, 11.3) uses `combine_side="img"` (`configs/model/clip_base.yaml`, unoverridden by `scripts/run_condition_freeze_ablation.sh`) — `CoSiRModel.forward()` fuses the trainable per-sample condition into the **image** embedding only (`self.combiner(img_emb, img_full, lbl_emb)`); the text side passes through `other_proj`, an identity-initialized linear layer the condition never touches. Under this architecture, "trained vs. frozen" is *by construction* only ever a difference in the image-side representation. That alone would predict exactly the asymmetry seen here: i2t (query = image, the condition-bearing side) moves with training while t2i (query = text, condition-blind) sits at the noise floor (11.1), and nodes whose defining graph structure is image-derived (`img_only_only`) show the most training-induced movement, since that is the only side training can act on at all. Under this reading, the `img_only_only` effect may be less a story about false-transitivity correction and more a fairly direct consequence of *where the condition is allowed to act* — real, but architectural rather than graph-topological.

**Resolved (2026-08-27): a direct `combine_side="txt"` replication was run** — see `docs/reports/2026-08-27_combine_side_txt_replication.md`. The two candidate explanations split apart cleanly: the bridge-subgroup concentration effect **is** architectural and flips exactly as this second explanation predicts — under `combine_side="txt"`, `txt_only_only` (not `img_only_only`) becomes the outsized subgroup, in all 3 replication seeds — so both explanations were partly right (the graph-topology story explains *why* a one-sided subgroup is affected at all; the `combine_side` story explains *which* one-sided subgroup it is). The much larger i2t-vs-t2i retrieval asymmetry from 11.1, however, did **not** flip to t2i under `combine_side="txt"` — it stayed on i2t, just ~11× smaller — so that headline effect is not explained by `combine_side` alone. See the linked report for the full breakdown.
...
- ~~All 6 runs use `combine_side="img"`... this experiment cannot distinguish them.~~ **Resolved 2026-08-27:** a `combine_side="txt"` replication (`docs/reports/2026-08-27_combine_side_txt_replication.md`) found the subgroup effect flips to `txt_only_only`, confirming the two explanations are complementary rather than competing (see the added Interpretation paragraph above) — but the much larger i2t-vs-t2i asymmetry did not flip, so that part remains unexplained.
