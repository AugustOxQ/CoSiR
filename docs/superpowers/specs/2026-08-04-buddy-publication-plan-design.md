...
| **C1a** — RedCaps' aggregate ~20× lift (C1) is not evenly distributed: per-subreddit lift spans 4.4×–671× across 159 qualifying subreddits, but none of three candidate properties (sample count, caption diversity, visual homogeneity) meaningfully predicts a subreddit's position on that range (Pearson r = −0.328, −0.219, +0.204 respectively, all n=159) | full per-subreddit `subreddit_lift` breakdown vs. closed-form pairwise-similarity properties, 1 dataset | `2026-08-24_redcaps_subreddit_signal_correlates.md` |
...
| **4–6** | Experiment 8 (buddy-init encoder-pair ablation) if in scope per the Week-3 checkpoint: build/validate the 150k sweep first, extend to 300k if informative. Runs alongside Experiment 5/7, not sequentially before them. |
