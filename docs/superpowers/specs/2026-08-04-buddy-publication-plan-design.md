...
| **Stretch** | **ICLR / NeurIPS main track** | **Checked (Exp. 1 ran, C5) — gate not cleared as written.** Exp. 1 found a real effect, but not a *clean* win: RedCaps beats cleanly on both directions, Impressions splits (i2t wins, t2i loses), all four seed-replicated. The "clean win on ≥2/3 datasets" bar this tier required isn't met by a directionally-mixed result on one of only two datasets tested (COCO/Exp. 5 not run). Stretch tier stays closed unless COCO (Exp. 5) lands as a third, clean data point, or the Impressions t2i regression gets diagnosed and resolved. | Re-evaluate only if Exp. 5 (COCO) is pursued and lands clean, or if a follow-up explains/fixes the Impressions t2i split. Not committed to in the current timeline (§6). |
