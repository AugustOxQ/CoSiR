"""Run the Stage 2 learned two-teacher ArtELingo student pilot on a GPU.
...
REPORT_PATH = os.path.join(OUT_DIR, "learned_student_stage2_pilot_report.md")
...
class LearnedStudent(nn.Module):
    """The predeclared small 2-layer MLP (64-unit hidden layer) two-view student with a scalar fusion gate."""
...
def write_report(
    ...
    lines.extend([
        ...
        f"| Learned student, Stage 1 — linear heads (reference) | 0.1284 | 0.1289 | 0.2799 | 0.3014 |\n",
        f"| Learned student, Stage 2 (this run) | {final_metrics['emotion']['AMI']:.4f} | "
        f"{final_metrics['emotion']['V_measure']:.4f} | {final_metrics['genre']['AMI']:.4f} | "
        f"{final_metrics['genre']['V_measure']:.4f} |\n",
        "\n## Held-out generalization\n\n",
        "| signal | emotion AMI | emotion V-measure | genre AMI | genre V-measure |\n",
        "|---|---:|---:|---:|---:|\n",
        f"| Learned student, Stage 2 — TRAIN | {final_metrics['emotion']['AMI']:.4f} | "
        f"{final_metrics['emotion']['V_measure']:.4f} | {final_metrics['genre']['AMI']:.4f} | "
        f"{final_metrics['genre']['V_measure']:.4f} |\n",
        "| Learned student, Stage 1 — linear heads (reference) | 0.1095 | 0.1123 | 0.2901 | 0.4240 |\n",
        f"| Learned student, Stage 2 — HELD-OUT | {heldout_metrics['emotion']['AMI']:.4f} | "
    ...
    if verdict == "Real success":
        lines.append("The Stage 2 MLP-head student is a reportable win; further capacity increases are not warranted yet.\n")
