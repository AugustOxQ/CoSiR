"""Run the Stage 1 learned-student content-loss-weight sweep on a GPU.
...
REPORT_PATH = os.path.join(OUT_DIR, "learned_student_weight_sweep_pilot_report.md")
...
CURRENT_CONTENT_WEIGHT: float | None = None
...
def log(message: str) -> None:
    prefix = "" if CURRENT_CONTENT_WEIGHT is None else f"[weight={CURRENT_CONTENT_WEIGHT:.1f}] "
...
def heldout_trend(results: list[dict]) -> str:
    """Describe the observed held-out emotion-AMI shape without assuming monotonicity."""
    values = [result["heldout_metrics"]["emotion"]["AMI"] for result in results]
    ...
    if 0 < best_index < len(values) - 1:
        return f"peaks in the middle at content_weight={results[best_index]['content_weight']:.1f}"
    return "is non-monotonic without an interior peak"


def write_report(results: list[dict]) -> None:
    """Write the predeclared sweep report after every independently seeded run."""
    baseline = next(result for result in results if result["content_weight"] == 1.0)
    baseline_train = baseline["final_metrics"]
    baseline_heldout = baseline["heldout_metrics"]
    sanity_check = (
        abs(baseline_train["emotion"]["AMI"] - 0.1284) <= 0.005
        and abs(baseline_train["genre"]["AMI"] - 0.2799) <= 0.005
        and abs(baseline_heldout["emotion"]["AMI"] - 0.1095) <= 0.005
        and abs(baseline_heldout["genre"]["AMI"] - 0.2901) <= 0.005
    )
    heldout_successes = [
    ...
    selection_pool = heldout_successes if heldout_successes else results
    best = max(
        selection_pool,
        key=lambda result: (
    ...
    best_heldout = best["heldout_metrics"]
    best_beats_stage1 = (
        best_heldout["emotion"]["AMI"] > baseline_heldout["emotion"]["AMI"]
        and best_heldout["genre"]["AMI"] > baseline_heldout["genre"]["AMI"]
    )
    lines = [
        "# ArtELingo learned two-teacher student — content-loss-weight sweep\n\n",
        f"Generated automatically, {time.strftime('%Y-%m-%d %H:%M:%S')}.\n\n",
        "## Context\n\n",
        "Stage 1's equally weighted learned student was the strongest result so far, but its "
        "held-out emotion AMI missed the target. Its checkpoint trajectory showed the content "
        "gradient share at 0.7356 at epoch 5 before it settled toward about 0.50 by epoch 200, "
        "motivating this predeclared sweep of a higher content-loss weight while holding the "
        "architecture, data, diagnostics, and stopping rule fixed.\n\n",
        "## Sanity check\n\n",
    ]
    if sanity_check:
        lines.append(
            "The `content_weight=1.0` in-sweep sanity check reproduces Stage 1 within 0.005 AMI: "
            f"train emotion/genre AMI={baseline_train['emotion']['AMI']:.4f}/"
            f"{baseline_train['genre']['AMI']:.4f} (Stage 1: 0.1284/0.2799) and held-out "
            f"emotion/genre AMI={baseline_heldout['emotion']['AMI']:.4f}/"
            f"{baseline_heldout['genre']['AMI']:.4f} (Stage 1: 0.1095/0.2901).\n\n"
        )
    else:
        lines.append(
            "The `content_weight=1.0` in-sweep sanity check does not reproduce Stage 1 closely "
            "(within 0.005 AMI), which is a discrepancy requiring investigation: "
            f"train emotion/genre AMI={baseline_train['emotion']['AMI']:.4f}/"
            f"{baseline_train['genre']['AMI']:.4f} (Stage 1: 0.1284/0.2799) and held-out "
            f"emotion/genre AMI={baseline_heldout['emotion']['AMI']:.4f}/"
            f"{baseline_heldout['genre']['AMI']:.4f} (Stage 1: 0.1095/0.2901).\n\n"
    ...
    for result in results:
        ...
        for split, metrics in (("train", result["final_metrics"]), ("held-out", result["heldout_metrics"])):
            lines.append(
                f"| {result['content_weight']:.1f} | {split} | {metrics['emotion']['AMI']:.4f} | "
                f"{metrics['genre']['AMI']:.4f} | {result['verdict']} | "
                f"{final['content_gradient_share']:.4f} | {final['gate_mean']:.4f} | "
    ...
    if heldout_successes:
        cleared_weights = ", ".join(f"{result['content_weight']:.1f}" for result in heldout_successes)
        lines.append(
            f"Weight(s) {cleared_weights} clear it. The best successful point by summed held-out AMI "
            f"is content_weight={best['content_weight']:.1f}, with emotion/genre AMI="
            f"{best_heldout['emotion']['AMI']:.4f}/{best_heldout['genre']['AMI']:.4f}.\n\n"
        )
    else:
        lines.append(
            "No weight clears it. The best point by summed held-out AMI is "
            f"content_weight={best['content_weight']:.1f}, with emotion/genre AMI="
            f"{best_heldout['emotion']['AMI']:.4f}/{best_heldout['genre']['AMI']:.4f}.\n\n"
        )
    lines.extend([
        "## Held-out trend\n\n",
        "Held-out emotion AMI across content weights is "
        + ", ".join(
            f"{result['content_weight']:.1f}: {result['heldout_metrics']['emotion']['AMI']:.4f}"
            for result in results
        )
        + f"; it {heldout_trend(results)}. The corresponding held-out genre AMIs are "
        + ", ".join(
            f"{result['content_weight']:.1f}: {result['heldout_metrics']['genre']['AMI']:.4f}"
            for result in results
        )
        + ".\n\n",
        "## Conclusion\n\n",
    ])
    if best_beats_stage1:
        lines.append(
            f"Content_weight={best['content_weight']:.1f} beats Stage 1's held-out results on both "
            "axes simultaneously and is the new best result in this investigation.\n"
        )
    elif not heldout_successes:
        emotion_gap = 0.1236 - best_heldout["emotion"]["AMI"]
        stage1_gap = 0.1236 - baseline_heldout["emotion"]["AMI"]
        if emotion_gap < stage1_gap:
            lines.append(
                "No weight clears the held-out Pareto bar, but the best point narrows Stage 1's "
                f"held-out emotion-AMI shortfall from {stage1_gap:.4f} to {emotion_gap:.4f}; this "
                "is reported as a gap reduction, not a Pareto success.\n"
            )
        else:
            lines.append(
                "No weight clears the held-out Pareto bar, and the sweep makes no material "
                "improvement over Stage 1's unweighted held-out emotion-AMI shortfall.\n"
            )
    else:
        lines.append(
            "A weight clears the held-out Pareto bar, but the selected best successful point does "
            "not beat Stage 1 on both axes simultaneously.\n"
        )
...
def main() -> None:
    ...
    results = []
    for content_weight in CONTENT_LOSS_WEIGHTS:
        global CURRENT_CONTENT_WEIGHT
        ...
        for epoch in range(1, MAX_EPOCHS + 1):
            ...
            total_loss = content_weight * content_loss + affect_loss

            gradient_share = None
            if epoch % CHECKPOINT_EVERY == 0:
                optimizer.zero_grad(set_to_none=True)
                (content_weight * content_loss).backward(retain_graph=True)
        ...
        results.append(
            {
                ...
                "heldout_metrics": {"emotion": heldout_emotion_metrics, "genre": heldout_genre_metrics},
                "verdict": verdict,
            }
        )
        log(
            f"Completed run; verdict={verdict}; train emotion AMI={emotion_metrics['AMI']:.4f}, "
            f"genre AMI={genre_metrics['AMI']:.4f}; held-out emotion AMI={heldout_emotion_metrics['AMI']:.4f}, "
            f"genre AMI={heldout_genre_metrics['AMI']:.4f}."
        )

    CURRENT_CONTENT_WEIGHT = None
    write_report(results)
    log(f"Wrote weight-sweep report to {REPORT_PATH}.")
