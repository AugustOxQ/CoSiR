...
def train_cosir_phase2(cfg, logger):
    ...
    pre_trained_representatives = get_representatives_polar_grid(
    ...
    for epoch in range(cfg.train.epochs_2):
        ...
        for batch_idx, batch in enumerate(tqdm(train_loader)):
            ...
            # Start masking
            rand_img = torch.rand(len(img_features))
            rand_txt = torch.rand(len(txt_features))

            # Build mask
            img_input = img_features.clone()
            txt_input = txt_features.clone()

            # Mask input
            mask_img = rand_img < mask_ratio
            mask_txt = rand_txt < mask_ratio

            # But we can only mask one of them, not both

            both_masked = mask_img & mask_txt
            if both_masked.any():
                # 随机保留一个
                keep_img = torch.rand(both_masked.sum()) > 0.5
                mask_img[both_masked] = keep_img
                mask_txt[both_masked] = ~keep_img

            # Apply mask
            img_for_predictor = img_input if not mask_img.any() else None
            txt_for_predictor = txt_input if not mask_txt.any() else None

            # Full pair
            idx_both = ~mask_img & ~mask_txt
            if idx_both.any():
                pred_cond_both, logits_both = model.predict_condition(
                    img_features[idx_both],
                    txt_features[idx_both],
                    "imgtxt",
                    pre_trained_representatives_device,
                )
            ...
            # Mask img
            idx_mask_img = mask_img & ~mask_txt
            if idx_mask_img.any():
                pred_cond_img, logits_img = model.predict_condition(
                    None,
                    txt_features[idx_mask_img],
                    "img",
                    pre_trained_representatives_device,  # masked
                )
                ...
                loss_dict_img = criteria_2(
                    logits_img,
                    pred_cond_img,
                    img_features[idx_mask_img],
                    txt_features[idx_mask_img],
                    batch_labels[idx_mask_img],
                    comb_emb_img,
                    epoch,
                )
            ...
            # Mask txt
            idx_mask_txt = ~mask_img & mask_txt
            if idx_mask_txt.any():
                pred_cond_txt, logits_txt = model.predict_condition(
                    img_features[idx_mask_txt],
                    None,
                    "txt",
                    pre_trained_representatives_device,
                )
                comb_emb_txt = model.combine(
                    txt_features[idx_mask_txt],
                    None,
                    pred_cond_txt,
                    epoch=epoch,
                    return_label_proj=False,
            ...
            diff_img = (
                torch.nn.functional.mse_loss(
                    comb_emb_img, comb_emb_origin[idx_mask_img, :]
                )
                if idx_mask_img.any()
                else 0
            )
            diff_txt = (
                torch.nn.functional.mse_loss(
                    comb_emb_txt, comb_emb_origin[idx_mask_txt, :]
                )
                if idx_mask_txt.any()
                else 0
            )
            diff_imgtxt = (
                torch.nn.functional.mse_loss(
                    comb_emb_both, comb_emb_origin[idx_both, :]
                )
                if idx_both.any()
                else 0
            )
            ...
            # Check magnitude
            magnitude_img = (
                torch.norm(pred_cond_img, p=2, dim=1).mean()
                if idx_mask_img.any()
                else 0
            )
            magnitude_txt = (
                torch.norm(pred_cond_txt, p=2, dim=1).mean()
                if idx_mask_txt.any()
                else 0
            )
            magnitude_imgtxt = (
                torch.norm(pred_cond_both, p=2, dim=1).mean() if idx_both.any() else 0
            )

            # Check diversity
            diversity_img = pred_cond_img.std(dim=0).mean() if idx_mask_img.any() else 0
            diversity_txt = pred_cond_txt.std(dim=0).mean() if idx_mask_txt.any() else 0
            diversity_imgtxt = pred_cond_both.std(dim=0).mean() if idx_both.any() else 0
            ...
            batch_log_dict.update(
                {
                    ...
                    "train_3/diff_embedding_img": (
                        diff_img.item() if idx_mask_img.any() else 0
                    ),
                    "train_3/diff_embedding_txt": (
                        diff_txt.item() if idx_mask_txt.any() else 0
                    ),
                    "train_3/diff_embedding_imgtxt": (
                        diff_imgtxt.item() if idx_both.any() else 0
                    ),
                    "train_3/magnitude_img": (
                        magnitude_img.item() if idx_mask_img.any() else 0
                    ),
                    "train_3/magnitude_txt": (
                        magnitude_txt.item() if idx_mask_txt.any() else 0
                    ),
                    "train_3/magnitude_imgtxt": (
                        magnitude_imgtxt.item() if idx_both.any() else 0
                    ),
                    "train_3/diversity_img": (
                        diversity_img.item() if idx_mask_img.any() else 0
                    ),
                    "train_3/diversity_txt": (
                        diversity_txt.item() if idx_mask_txt.any() else 0
                    ),
                    "train_3/diversity_imgtxt": (
                        diversity_imgtxt.item() if idx_both.any() else 0
                    ),
                }
            )
        ...
        with torch.no_grad():
            ...
            test_results_detailed = evaluator.evaluate_test(
                ...
                use_oracle=False,
                pre_trained_representatives=pre_trained_representatives_device,
            )
