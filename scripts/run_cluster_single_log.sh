...
# 使用lr_safe避免文件名问题，如果设置了CUDA_VISIBLE_DEVICES，也添加到目录名避免冲突
if [ "$GPU_ID" != "auto" ] && [ "$GPU_ID" != "cpu" ]; then
    EXP_LOG_DIR="/local/wding/res/CoSiR_Experiment/logs_${lr_safe}_gpu${GPU_ID}"
else
    EXP_LOG_DIR="/local/wding/res/CoSiR_Experiment/logs_${lr_safe}"
fi
...
echo "  Log Directory: $EXP_LOG_DIR"
echo "=========================================="

# 注意：当设置了CUDA_VISIBLE_DEVICES时，PyTorch会将可见的GPU映射为cuda:0
# 所以device=cuda:0是正确的，它会使用CUDA_VISIBLE_DEVICES指定的GPU
python main_cosir.py dataset=coco_cluster optimizer.lr=$input_lr device=cuda:0 wandb.name="exp_lr_${lr_safe}" wandb.notes="$EXP_NOTE_PREFIX with lr $input_lr" > $EXP_LOG_DIR/lr_${lr_safe}.txt 2>&1 &
PID=$!
echo "Started experiment (lr=$input_lr) on GPU ${GPU_ID:-auto}, PID=$PID"
echo "Log file: $EXP_LOG_DIR/lr_${lr_safe}.txt"